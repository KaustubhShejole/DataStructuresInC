...
P_Node* SortByTravelDateHelper(Node* lptr,int pid,P_Node* zlptr);
P_Node* SortTravelDateHelper(Tree_Node*  root,int pid,P_Node* zlptr)
{
    if(root!=NULL)
    {
        zlptr = SortTravelDateHelper(root->left,pid,zlptr);
        zlptr = SortByTravelDateHelper(root->lptr,pid,zlptr);
        zlptr = SortTravelDateHelper(root->right,pid,zlptr);
    }
    return zlptr;
}
P_Node* SortByTravelDateHelper(Node* lptr,int pid,P_Node* zlptr)
...
void SortByTravelDate()
{
    ...
    if (temp!=-1)
    {
        zlptr = SortTravelDateHelper(root,temp,zlptr);
    }
